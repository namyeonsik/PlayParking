...
public class ReserveCancelController {
	...
	public ModelAndView getMaplist(HttpServletRequest request,
		...
		if (session.getAttribute("memcheck") == null) {
			...
			List<ReservationDTO> reservationlist = service.selectReservation3(
					mid, start_num, last_num);
			List<ReservationDTO> reservationlist2 = service.selectByaid12(mid);

			mv.addObject("reservationlist", reservationlist);
			mv.addObject("reservationlistsize", reservationlist2.size());
			mv.setViewName("/members/myparking/reservecancel");
			return mv;
		}
	}
